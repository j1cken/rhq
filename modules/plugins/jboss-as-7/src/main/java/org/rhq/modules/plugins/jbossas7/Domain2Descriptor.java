...
public class Domain2Descriptor {
	...
	String path = arg;
	path = path.replaceAll("/", ","); // Allow path from jboss-admin.sh's
					  // pwd command
	...
	for (Object key : keys) {
		...
		if (ptype == Type.OBJECT) {
			...
			createMetricEntry(indent, (Map<String, Object>) myEntry.getValue(),
			    entryName + ":" + myEntry.getKey(), getTypeFromProps(myMap));
		    }
		} else {
		    if (!accessType.equals("metric")) {
			continue;
		    }
		    createMetricEntry(indent, props, entryName, ptype);
		}
	...
	Map<String, Object> emapEntryValue = (Map<String, Object>) emapEntry.getValue();
	StringBuilder sb = generateProperty(indent, emapEntryValue, getTypeFromProps(emapEntryValue),
